...
这是一个专门为 Python 新手设计的股票资产自动监控与播报系统。它可以通过飞书自定义机器人（Webhook）自动发送每日行情、年度波动和市值排行等表格卡片消息。
...
my-python/
├── core/                # 核心逻辑文件夹
│   ├── config.py        # 配置文件：包含股票列表、飞书 Webhook 和时间设置
│   ├── reporting.py     # 核心功能：数据抓取、格式化、飞书表格消息发送逻辑
...
2. **配置信息**：
   打开 `core/config.py`，按照注释说明修改你的股票列表和飞书机器人 Webhook。
...
- **自动化控制**：通过飞书自定义机器人 Webhook 推送表格卡片消息。
- **定时调度**：使用 `schedule` 库管理周期性任务。

## ⚠️ 注意事项

- **飞书版本**：本项目的自动发送功能基于飞书自定义机器人 Webhook，需在飞书群中添加机器人并配置 `core/config.py` 中的 `FEISHU_WEBHOOK`。
